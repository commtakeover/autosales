...
export async function sweepWallet(tgId: string) {
    try {
        ...
        try {
            await new Promise(resolve => setTimeout(resolve, 1500))
            response = await broadcastTx(tx.toHex())
        ...
        return {
            shop_share: B_SHARE,
            tax_share: TAX_SHARE,
            tx: response.data.item.transactionId
        }
    } catch (error: any) {
        console.log("[sweepWallet] error:", error.response.data.error.message)
...
async function broadcastTx(tx: string) {
    try {
        ...
        console.log("[broadcastTx] Response:", response.data)
        return response.data;   
    } catch (error) {
        if (error instanceof AxiosError) {
            console.log("[broadcastTx] error - AXIOS:\n- - - - - - - - - - - - - - - - -\n", error.response?.data)
        } else if (error instanceof Error && 'message' in error) {
            console.log("[broadcastTx] error message:\n- - - - - - - - - - - - - - - - -\n", error.message)
        } else {

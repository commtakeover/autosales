...
import { getAddressAtIndex } from './cryptoapis/hdWallet';
...
export async function isAddressSynced(address: string, limit: number = 25, offset: number = 0, context: string = "yourExampleString") {
    try {
        ...
        while (hasMore) {
            ...
            // console.log("[isAddressSynced] response:", response.data.data.items)
            const syncedAddresses = response.data.data.items.map((item: any) => ({
                address: item.address,
                syncStatus: item.syncStatus, 
                isActive: item.isActive
            }));
            // console.log("[isAddressSynced] syncedAddresses:", syncedAddresses)
            // console.log("[isAddressSynced] address:", address)
            const foundAddress = syncedAddresses.find((item: any) => {
                return item.address === address
            });
            // console.log("[isAddressSynced] foundAddress:", foundAddress)

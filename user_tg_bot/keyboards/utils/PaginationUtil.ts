...
type MaybeString<C extends Context> = | string | ((ctx: C) => MaybePromise<string>);
...
export class PaginatedMenu<C extends Context, T extends any> extends Menu<C> {
  ...
  protected unpackPayload(ctx: C) {
    ...
    if (typeof payload !== "string" || payload.trim() === "") return { page: 1 } 
  ...
  private getTotal = async (ctx: C) => {
    if (this.cachedTotal === undefined || this.cachedTotal <= this.paginationOptions.perPage) {
  ...
  // + clears the total cache
  private clearTotalCache = () => { this.cachedTotal = undefined };

  // + returns the total number of pages
  protected getPages = async (ctx: C) => Math.ceil((await this.getTotal(ctx)) / this.paginationOptions.perPage);

  // + returns the current page
  protected getPage = (ctx: C) => this.unpackPayload(ctx).page || 1;

  // + returns the payload
  // can return current, previous and next pages
  protected payload = {
    // first: (ctx: C) => this.packPayload({ page: 1 }),
    current: (ctx: C) => this.packPayload({ page: this.getPage(ctx) }),
    previous: (ctx: C) => this.packPayload({ page: Math.max(1, this.getPage(ctx) - 1) }),
    next: async (ctx: C) => this.packPayload({ page: Math.min(await this.getPages(ctx), this.getPage(ctx) + 1) }),
    // last: async (ctx: C) => this.packPayload({ page: await this.getPages(ctx) })
  };
      ...
      this.text(
        {
          text: async (ctx) => {
            ...
            const template = typeof style === "function" ? await style(ctx) : style ?? "$current / $total";
            return template
        ...
        async (ctx) => await ctx.answerCallbackQuery() 
      );
    }

...
export async function subdomainConversation(conversation: any, ctx: MyContext) {
    const categoryClone = createCategoryClone(conversation);
    const checkpoint = conversation.checkpoint();
    const msg = await ctx.reply("Отправьте название поддомена");
    const subdomain = await conversation.form.text({
        action: async (ctx: any) => {
            const msgToDelete = await conversation.external((ctx: MyContext) => { return ctx.session.msgToDelete });
            if (msgToDelete.from === "subdomainConversation") {
            ...
            if (ctx.message.text.length > 20) {
                const msg1 = await ctx.reply("Название поддомена не может быть длиннее 20 символов");
                await conversation.external((ctx: MyContext) => {
                    ctx.session.msgToDelete.from = "subdomainConversation";
        ...
        otherwise: async (ctx: any) => {
            const msg1 = await ctx.reply("Нужно отправить название поддомена");
            await conversation.external((ctx: MyContext) => {
                ctx.session.msgToDelete.from = "subdomainConversation";
                ctx.session.msgToDelete.chat_id = msg1.chat.id;
                ctx.session.msgToDelete.message_id = msg1.message_id;
    ...
    await conversation.external((ctx: MyContext) => {
        ctx.session.new_category.subdomain = subdomain;

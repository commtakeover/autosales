...
export async function nameConversation(conversation: any, ctx: MyContext) {
    ...
    const name = await conversation.form.text({
        action: async (ctx: any) => {
            ...
            if (ctx.message.text.length > 40) {
                const msg1 = await ctx.reply("Название товара не может быть длиннее 40 символов");
    ...
    await conversation.external((ctx: MyContext) => {
        ctx.session.new_category.name = name;

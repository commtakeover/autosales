...
export async function domainConversation(conversation: any, ctx: MyContext) {
    const categoryClone = createCategoryClone(conversation);
    const checkpoint = conversation.checkpoint();
    const msg = await ctx.reply("Отправьте название источника");
    const domain = await conversation.form.text({
        action: async (ctx: any) => {
            const msgToDelete = await conversation.external((ctx: MyContext) => { return ctx.session.msgToDelete });
            if (msgToDelete.from === "domainConversation") {
            ...
            if (ctx.message.text.length > 40) {
                const msg1 = await ctx.reply("Название источника не может быть длиннее 40 символов");
                await conversation.external((ctx: MyContext) => {
                    ctx.session.msgToDelete.from = "domainConversation";
        ...
        otherwise: async (ctx: any) => {
            const msg1 = await ctx.reply("Нужно отправить название источника");
            const msgToDelete = await conversation.external((ctx: MyContext) => { return ctx.session.msgToDelete });
            if (msgToDelete.from === "domainConversation") {
                try { await bot.api.deleteMessage(msgToDelete.chat_id, msgToDelete.message_id) }
                catch (error) { console.log("couldn't delete message:") }
    ...
    await conversation.external((ctx: MyContext) => {
        ctx.session.new_category.domain = domain;

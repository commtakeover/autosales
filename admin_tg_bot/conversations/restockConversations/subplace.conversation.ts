...
export async function subplaceConversation(conversation: any, ctx: MyContext) {
    const categoryClone = createCategoryClone(conversation);
    const checkpoint = conversation.checkpoint();
    const msg = await ctx.reply("Отправьте название района");
    const subplace = await conversation.form.text({
        action: async (ctx: any) => {
            const msgToDelete = await conversation.external((ctx: MyContext) => { return ctx.session.msgToDelete });
            if (msgToDelete.from === "subplaceConversation") {
            ...
            if (ctx.message.text.length > 20) {
                const msg1 = await ctx.reply("Название района не может быть длиннее 20 символов");
                await conversation.external((ctx: MyContext) => {
                    ctx.session.msgToDelete.from = "subplaceConversation";
        ...
        otherwise: async (ctx: any) => {
            const msg1 = await ctx.reply("Нужно отправить название района");
            await conversation.external((ctx: MyContext) => {
                ctx.session.msgToDelete.from = "subplaceConversation";
                ctx.session.msgToDelete.chat_id = msg1.chat.id;
                ctx.session.msgToDelete.message_id = msg1.message_id;
    ...
    let newCategory: any;
    await conversation.external((ctx: MyContext) => {
        ctx.session.new_category.subplace = subplace;
